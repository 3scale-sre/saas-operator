...
// NewAPIOptions returns Zync API options given saasv1alpha1.ZyncSpec
func NewAPIOptions(spec saasv1alpha1.ZyncSpec) pod.Options {
	opts := pod.Options{}
	...
	opts.AddEnvvar("RAILS_MAX_THREADS").Unpack(spec.Config.Rails.MaxThreads)
	...
	opts.AddEnvvar("BUGSNAG_API_KEY").AsSecretRef(ZyncSecret).WithSeedKey(seed.ZyncBugsnagApiKey).
